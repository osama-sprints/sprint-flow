...
- **Long-term memory** via mem0 + Qdrant — semantic search per user, cache-backed
...
| [Database & Migrations](docs/database.md) | Schema, Alembic migrations |
...
mem0 runs in-process and persists its embeddings to an external Qdrant instance — there is no mem0 cloud account or API key. You need a working `OPENAI_API_KEY` (used for fact extraction + embeddings) plus `QDRANT_URL` and `QDRANT_API_KEY`. See [docs/memory.md](docs/memory.md) for details.
...
- Verify `QDRANT_URL` and `QDRANT_API_KEY` are set and the Qdrant instance is reachable
